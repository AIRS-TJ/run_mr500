...
MAPS_DIR = HOME_DIR + '/.robuster/robuster_101/'
...
class ServiceManager:
    def __init__(self):
        ...
        rospy.Service('/robuster/mapping/save_3dmap', MappingSave3D, self.handleMappingSave3DMap)
    ...
    def handleMappingSave3DMap(self, req):
        if self.mapping_started or self.navigation_started:
            return MappingSave3DResponse(-1, "Save3DMap Control failed because mapping or naving is running.")
        else:
            print(req.msg)
            if req.msg:         
                result = self.startService('mappingsave')
                return MappingSave3DResponse(result, "Save3DMap start success.")
            else:               
                result = self.stopService('mappingsave')
                return MappingSave3DResponse(result, "Save3DMap stop success.")
    ...
    def handleMappingSelect(self, req):
        ...
        subdirs = dirs[0]
        if req.filename not in subdirs:
            return MappingSelectResponse(-8, 'Selected ' + req.filename + 'is not exist')
